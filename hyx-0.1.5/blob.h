...
static inline size_t blob_length(struct blob const *blob) { return blob->len; }

byte const *blob_lookup(struct blob const *blob, size_t pos, size_t *len);

static inline byte blob_at(struct blob const *blob, size_t pos) { return *blob_lookup(blob, pos, NULL); }
